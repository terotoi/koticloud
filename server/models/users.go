...
func (o *User) Meta(mods ...qm.QueryMod) metumQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadMeta(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`meta`),
		qm.WhereIn(`meta.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.User appropriately.
func (o *User) AddMeta(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Metum) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID

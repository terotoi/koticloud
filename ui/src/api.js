 ...
 * @param {function} error - function(message) called on error
 */
function _queryNode(id, authToken, success, error) {
 ...
 * @param {function} error - function(message) called on error
 */
function _listDir(id, authToken, success, error) {
...
class Uploader {
  ...
  upload(file, { filename, nodeID }) {
    ...
    return axios.post(this.props.url, fd, {
      ...
      const node = new Node(r.data[0])
      this.props.done(node)
    }).catch((err) => {
      console.log("Error:", err)
      if (this.props.error)
